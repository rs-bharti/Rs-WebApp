...
import { getCustomers, getSuppliers, getMasterBranchMasters, getPaymentMethods, getCustomerLedger, getSupplierLedger } from '../../../api/masters';
...
const ReceiptVoucherForm = () => {
  ...
  const COLUMNS = [
    ...
    { key: 'particular',    label: 'Particulars', render: v => v.particularName || v.customer?.name || '—' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {[['customer', 'Customer', '/dashboard/master/customer'], ['supplier', 'Supplier', '/dashboard/master/supplier']].some(([, label]) => canAccessMaster(label)) && (
              <div className="relative" ref={masterPopupRef}>
                ...
                {masterPopup && (
                  <div className="absolute right-0 top-7 z-50 bg-white border border-stone-200 rounded-xl shadow-lg py-1 min-w-[160px] animate-in fade-in zoom-in-95 duration-100">
                    {[['customer', 'Customer', '/dashboard/master/customer'], ['supplier', 'Supplier', '/dashboard/master/supplier']].filter(([, label]) => canAccessMaster(label)).map(([key, label, path]) => (

  ...
  return (
    ...
    <section className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => navigate('/dashboard/master/customer')} className="text-rs-text-muted hover:text-rs-text-primary bg-rs-text-primary/10 hover:bg-rs-text-primary/20 rounded p-0.5 transition-all cursor-pointer" title="Go to Customer Master"><Plus className="w-4 h-4" /></button>
            </div>
            <SelectSearch
              value={partyKey}
      ...
      <VoucherListModal
        ...
        onUpdate={async (id, data) => { const u = await updateSalesReturnVoucher(id, data); setVouchers(p => p.map(v => v.id === id ? { ...v, ...u } : v)); emitDataChange(); }}

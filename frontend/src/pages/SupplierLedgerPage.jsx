...
const fetchSupplierLedger = async (supplierId) => {
  const res = await fetch(`${API_URL}/api/vouchers/supplier-ledger/${supplierId}`, {
    headers: authHeaders(),
  });
...
const ItemsSubRow = ({ items, meta, source }) => {
  if (!items?.length) {
    // Payment: show payment method
    if (source === 'payment' && meta?.paymentMethod) {
      return (
...
// Main ledger row
const LedgerRow = ({ row, index }) => {
  const [expanded, setExpanded] = useState(false);
  const hasItems = row.items?.length > 0;
  const hasDetail = hasItems || (row.source === 'payment' && row.meta?.paymentMethod);
...
const SupplierLedgerPage = () => {
  ...
  const periodRows = dateFiltered.map(r => {
    // Payment is stored as CR but shown in DR column — must subtract
    const inCR = r.type === 'CR' && r.source !== 'payment';
  ...
  const summary = {
    totalPurchases:       periodRows.filter(r => r.source === 'purchase').reduce((s, r) => s + r.amount, 0),
    totalPurchaseReturns: periodRows.filter(r => r.source === 'purchase_return').reduce((s, r) => s + r.amount, 0),
    totalPayments:        periodRows.filter(r => r.source === 'payment').reduce((s, r) => s + r.amount, 0),
  };
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 animate-in fade-in duration-300">
      ...
      {!selectedId && !loading && (
        <div className="text-center py-24 bg-white border border-dashed border-stone-200 rounded-xl flex flex-col items-center gap-3 text-stone-400">
          <Truck className="w-12 h-12 text-stone-200" />
          <p className="text-sm">Select a supplier to view their complete ledger</p>
        </div>
      )}
            ...
            <div className="bg-white border border-stone-200 rounded-xl px-5 py-4 shadow-sm">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-stone-800 text-base">{supplier.name}</p>
                  <div className="flex flex-wrap gap-x-5 gap-y-1 mt-1.5 text-xs text-stone-500">
                    ...
                    {(supplier.cityName || supplier.stateName) && (
                      <span className="flex items-center gap-1">
                        <MapPin className="w-3 h-3" />
                        {[supplier.cityName, supplier.stateName, supplier.countryName].filter(Boolean).join(', ')}
                      </span>
                    )}
                    {supplier.gstNo && (
                      <span className="flex items-center gap-1"><FileText className="w-3 h-3" />GST: {supplier.gstNo}</span>
                    )}
                  </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-white border border-amber-100 rounded-xl p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-semibold text-stone-400 uppercase tracking-wider">Purchases</p>
              </div>
              <p className="text-xl font-bold text-stone-800 tabular-nums">₹{fmt(summary.totalPurchases)}</p>
            </div>
            ...
            <div className="bg-white border border-purple-100 rounded-xl p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-semibold text-stone-400 uppercase tracking-wider">Purch. Returns</p>
              </div>
              <p className="text-xl font-bold text-stone-800 tabular-nums">₹{fmt(summary.totalPurchaseReturns)}</p>
            </div>
            ...
            <div className="bg-white border border-emerald-100 rounded-xl p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-semibold text-stone-400 uppercase tracking-wider">Payments Made</p>
              </div>
              <p className="text-xl font-bold text-stone-800 tabular-nums">₹{fmt(summary.totalPayments)}</p>
            </div>
            ...
            <table className="w-full text-sm min-w-[700px]">
              ...
              <tbody className="divide-y divide-stone-100">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((row, i) => (
                    <LedgerRow key={row.id ?? i} row={row} index={i} />
                  ))
                )}
              </tbody>
              {filtered.length > 0 && (
